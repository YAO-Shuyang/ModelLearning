...
class CustomGraphEnv(GraphEnv):
    """GraphEnv wrapper using custom objects/edges.

    Parameters
    ----------
    objects
        Dict with "rewards": {state_id: reward}.
    edges
        Transition table edges[s][a] = s_next.
    obs_type
        Use GraphObservation.index for integer observations.
    seed
        Random seed.
    use_noop
        If True, adds an extra no-op action.
    ...
    def __init__(
        ...
        self._custom_objects = objects
        self._custom_edges = edges
        self.use_noop = int(use_noop)
        super().__init__(template="linear", obs_type=obs_type, seed=seed)

    def generate_layout(self, template) -> None:
        """Override template generation with custom graph."""
        self.template_objects = self._custom_objects
        self.edges = self._custom_edges
        action_size = max(len(e) for e in self.edges)
