...
CP_DSP2 = {
    0: np.array([1,2,14,13,25,37,49,61,73,74,86,85,97,98,110,109,121,133,134,135,123,111,99,100,88,76,75,63,64,65,53,52,40,28,29,17,5,6,7,19,18,30,31,43,44,32,33,21,9,10,11,12,24,23,22,34,46,45,57,69,68,67,55,54,66,78,77,89,101,102,114,115,116,104,103,91,79,80,92,93,105,106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
    1: np.array([19,18,30,31,43,44,32,33,21,9,10,11,12,24,23,22,34,46,45,57,69,68,67,55,54,66,78,77,89,101,102,114,115,116,104,103,91,79,80,92,93,105,106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
    2: np.array([23,22,34,46,45,57,69,68,67,55,54,66,78,77,89,101,102,114,115,116,104,103,91,79,80,92,93,105,106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
    3: np.array([106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
    4: np.array([113,112,124,125,126,138,137,136,135,123,111,99,100,88,76,75,63,64,65,53,52,40,28,29,17,5,6,7,19,18,30,31,43,44,32,33,21,9,10,11,12,24,23,22,34,46,45,57,69,68,67,55,54,66,78,77,89,101,102,114,115,116,104,103,91,79,80,92,93,105,106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
    5: np.array([62,50,51,52,40,28,29,17,5,6,7,19,18,30,31,43,44,32,33,21,9,10,11,12,24,23,22,34,46,45,57,69,68,67,55,54,66,78,77,89,101,102,114,115,116,104,103,91,79,80,92,93,105,106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
    6: np.array([139,127,128,140,141,129,117,105,106,94,82,70,71,59,60,72,84,83,95,96,108,120,119,131,130,142,143,144], dtype = np.int64),
}
...
def get_metric(maze_id: int):
    """This function loads a precomputed distance matrix of 12x12 maze
    i.e., the ij entry of the matrix is the shortest path length from node 
    i to node j in the maze.
    
    The users can define their own metric by changing the code below. 
    For example, they can define maze_id = 3 and implement ways to upload
    their own files.
    """
    if maze_id == 1:
        with open(os.path.join(os.path.dirname(__file__), 'D12_1.pkl'), 'rb') as f:
            metric = pickle.load(f)
    elif maze_id == 2:
        with open(os.path.join(os.path.dirname(__file__), 'D12_2.pkl'), 'rb') as f:
            metric = pickle.load(f)
    elif maze_id == 3:
        raise NotImplementedError(
            "Users can implement their own way to load the metric for maze_id = 3."
        )
    else:
        raise ValueError("maze_id should be 1, 2, or 3.")
...
def check_relation(
    ...
    x1, y1 = (node_1 - 1) % NXBIN, (node_1 - 1) // NYBIN
    x2, y2 = (node_2 - 1) % NXBIN, (node_2 - 1) // NYBIN
    dx = x2 - x1
    dy = y2 - y1
    ...
    if np.where(actions == -1)[0].size > 0:
        raise ValueError(
            f"Nodes {node_1[np.where(actions == -1)[0]]} and "
            f"{node_2[np.where(actions == -1)[0]]} are not adjacent."
        )
...
class MazeEnv(object):
    def __init__(
        self, 
        graph: Dict[int, List[int]] = maze1_graph, 
        metric: np.ndarray = get_metric(maze_id=1),
        start_node: int = 1, 
        goal_node: int = 144
    ):
        self.graph = graph
        self.n_nodes = len(graph)
        self.legal_actions = np.zeros((self.n_nodes, 4), np.int64)-1
        ...
        self._init_familiar_transitions()
        self._init_novel_transitions()
        self._init_sensory()
    ...
    def _init_legal_actions(self, node: int):
        ...
        for node, neighbors in self.graph.items():
            for neighbor in neighbors:
                ...
                self.legal_actions[node-1, action] = 1
    ...
    def find_branch_ends(self, start_node: int, goal_node: int):
        ...
        for n in self.graph.keys():
            if n != start_node and n != goal_node and len(self.graph[n]) == 1:
                branch_end.append(n)
        self.branch_ends = np.array(branch_end, dtype=np.int64)
    ...
    def _init_novel_transitions(self):
        """Initialize action probability when novel"""
        self.SN = np.zeros((self.n_nodes, self.n_nodes), dtype=np.float64)
        for node in self.graph.keys():
            n_neighbors = len(self.graph[node])
            self.SN[node-1, np.asarray(self.graph[node])-1] = 1 / n_neighbors
    
    def _init_familiar_transitions(self):
        """Initialize action probability when familiar"""
        self.SF = np.zeros((self.n_nodes, self.n_nodes), dtype=np.float64)
        for node in self.graph.keys():
            for neighbor in self.graph[node]:
                if self.D[node-1, self.goal_node-1] > self.D[neighbor-1, self.goal_node-1]:
                    self.SF[node-1, neighbor-1] = 1
                    
    def _init_sensory(self):
        """
        0: Starting nodes
        ...
        5: Goal node
        """
        self.obs = np.zeros(self.n_nodes, dtype=np.int64)
        self.obs[self.start_node-1] = 0
        self.obs[self.goal_node-1] = 5
        self.obs[self.branch_ends-1] = 1
        for node in self.graph.keys():
            if node not in np.append(self.branch_ends, [self.start_node, self.goal_node]):
                n_neighbors = len(self.graph[node])
                if n_neighbors == 2:
                    neighbors = self.graph[node]
                    if np.abs(neighbors[0] - neighbors[1]) in [2, 2*NXBIN]:
                        self.obs[node-1] = 2
                    else:
                        self.obs[node-1] = 3
                else:
